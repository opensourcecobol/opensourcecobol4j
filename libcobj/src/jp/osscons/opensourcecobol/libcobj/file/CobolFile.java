...
public class CobolFile {
	...
	protected final static int COB_LOCK_EXCLUSIVE = 1;
	protected final static int COB_LOCK_MANUAL = 2;
	protected final static int COB_LOCK_AUTOMATIC = 3;
	protected final static int COB_LOCK_MULTIPLE = 1;
	protected final static int COB_LOCK_MASK = 0x7;
	 ...
	 */
	public static int invokeFun(int operate, Object f, AbstractCobolField key, CobolDataStorage rec, AbstractCobolField fnstatus,
		String openMode, String startCond, String readOpts) {
	...
	public void open(int mode, int sharing, AbstractCobolField fnstatus) {
		...
		try {
			...
			case 0:
				this.open_mode = (char)mode;
				if(this.flag_optional && was_not_exist) {
	...
	public int open_(String filename, int mode, int sharing) throws IOException {
		...
		try {
			...
			case COB_OPEN_OUTPUT:
				fp = FileChannel.open(Paths.get(filename), StandardOpenOption.WRITE, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
				break;
			case COB_OPEN_I_O:
				fp = FileChannel.open(Paths.get(filename),StandardOpenOption.READ, StandardOpenOption.WRITE);
				break;
			case COB_OPEN_EXTEND:
				fp = FileChannel.open(Paths.get(filename),StandardOpenOption.APPEND, StandardOpenOption.CREATE);
				break;
			}
		} catch(IOException e) {
			if(fp != null) {
		...
		if(!filename.startsWith("/dev/")) {
			try {
				...
				fl = fp.tryLock(0L, Long.MAX_VALUE,  lock_flag);
			} catch(NonWritableChannelException e) {
				fp.close(); return EBADF;
			} catch(ClosedChannelException e) {
				fp.close(); return COB_STATUS_61_FILE_SHARING;
	...
	public void read(AbstractCobolField key, AbstractCobolField fnstatus, int read_opts) {
		byte[] sbuff = new byte[3];
		String openMode = String.format("%02d", (int)this.last_open_mode);
		String readOpts = String.format("%02d", (int)read_opts);
		if(invokeFun(COB_IO_READ, this, key, null, fnstatus, openMode, null, readOpts) != 0) {
			for(int i=0; i<3; ++i) {
				sbuff[i] = 0;
			}
		...
		if(key == null) {
			if(this.flag_end_of_file && (read_opts & COB_READ_PREVIOUS) == 0) {
				saveStatus(COB_STATUS_46_READ_ERROR, fnstatus);
				return;
			}
			if(this.flag_begin_of_file && (read_opts & COB_READ_PREVIOUS) != 0 ) {
				saveStatus(COB_STATUS_46_READ_ERROR, fnstatus);
				return;
			}
		}
		...
		if(this.organization == COB_ORG_INDEXED) {
			...
			} else if((this.lock_mode & COB_LOCK_AUTOMATIC) != 0 && (read_opts & COB_READ_NO_LOCK) == 0) {
				read_opts |= COB_READ_LOCK;
			}
		} else {
			read_opts &= ~COB_READ_LOCK;
	...
	public void rewrite(AbstractCobolField rec, int opt, AbstractCobolField fnstatus) {
		...
		boolean read_done = this.flag_read_done;
		this.flag_read_done = false;
	...
	/**
	 * libcob/fileio.cのcob_exit_fileioの実装
	 * TODO 一部だけ実装したため残りを実装する
	 */
