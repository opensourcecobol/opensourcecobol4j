  ...
  private int cmpPacked(int n) {
    ...
    for (size = 0; size < 20; size++) {
      if (val1[size] != CobolDecimal.packedValue[size]) {
        if (sign < 0) {
          return Byte.toUnsignedInt(CobolDecimal.packedValue[size]) - Byte.toUnsignedInt(val1[size]);
        } else {
          return Byte.toUnsignedInt(val1[size]) - Byte.toUnsignedInt(CobolDecimal.packedValue[size]);
        }
      }
    }
    return 0;
