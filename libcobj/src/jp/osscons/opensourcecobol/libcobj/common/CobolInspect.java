...
public class CobolInspect {
	private final static int INSPECT_ALL = 0;
	private final static int INSPECT_LEADING = 1;
	private final static int INSPECT_FIRST = 2;
	private final static int INSPECT_TRAILING = 3;
	...
	private static void common(AbstractCobolField f1, AbstractCobolField f2, int type) throws CobolStopRunException {
		if(f1 == null) { f1 = CobolConstant.low; }
		if(f2 == null) { f2 = CobolConstant.low; }
		...
		if(type == INSPECT_TRAILING) {
			for(int i=len - f2.getSize(); i>=0; i--) {
				if(inspectData.getSubDataStorage(inspectStart + i).memcmp(f2.getDataStorage(), f2.getSize()) == 0) {
					int j;
	...
	public static void characters(AbstractCobolField f1) throws CobolStopRunException {
		...
		if(inspectReplacing != 0) {
			...
			if(n > 0) {
				...
				if(type == CobolFieldAttribute.COB_TYPE_NATIONAL || type == CobolFieldAttribute.COB_TYPE_NATIONAL_EDITED) {
					n = n / 2;
				}
				f1.addInt(n);
			}
	 ...
	 * @throws CobolStopRunException 
	 */
	public static void all(AbstractCobolField f1, AbstractCobolField f2) throws CobolStopRunException {
	 ...
	 * @throws CobolStopRunException 
	 */
	public static void leading(AbstractCobolField f1, AbstractCobolField f2) throws CobolStopRunException {		
	 ...
	 * @throws CobolStopRunException 
	 */
	public static void first(AbstractCobolField f1, AbstractCobolField f2) throws CobolStopRunException {
	 ...
	 * @throws CobolStopRunException 
	 */
	public static void trailing(AbstractCobolField f1, AbstractCobolField f2) throws CobolStopRunException {
		...
		if(type1 == CobolFieldAttribute.COB_TYPE_NATIONAL || type1 == CobolFieldAttribute.COB_TYPE_NATIONAL_EDITED) {
			...
			for(int j=0; j<f1.getSize(); j+=2) {
				for(int i=0; i<len; i+=2) {
					if(inspectMark[i] == -1 && inspectMark[i+1] == -1 &&
						f1.getDataStorage().getSubDataStorage(j).memcmp(inspectData.getSubDataStorage(inspectStart + i), 2) == 0) {
