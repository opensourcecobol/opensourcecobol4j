...
public class CobolNumericField extends AbstractCobolField {
    ...
    private void movePackedToDisplay(AbstractCobolField field) {
        ...
        for (int i = offset; i < end; ++i) {
            if (i % 2 == 0) {
                buff.setByte(
                        i - offset,
                        (byte) (((field.getDataStorage().getByte(i / 2) >> 4) & 0x0f) + 0x30));
            } else {
                buff.setByte(
                        i - offset, (byte) ((field.getDataStorage().getByte(i / 2) & 0x0f) + 0x30));
            }
        }
        this.storeCommonRegion(
    ...
    public int numericCompareTo(AbstractCobolField field) {
        ...
        if (attr2.isTypeNumericDisplay()) {
            ...
            if (attr1.isFlagHaveSign()
                    && !attr1.isFlagSignLeading()
                    && attr1.isFlagSignSeparate()) {
                lastIndex1 = size1 - 2;
            } else {
                lastIndex1 = size1 - 1;
            }
            final int lastIndex2;
            if (attr2.isFlagHaveSign()
                    && !attr2.isFlagSignLeading()
                    && attr2.isFlagSignSeparate()) {
                lastIndex2 = size2 - 2;
            } else {
                lastIndex2 = size2 - 1;
            }
