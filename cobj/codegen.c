...
static void joutput_goto_1(cb_tree x) {
  joutput_line("if(true) return Optional.of(contList[%d]);",
               find_label_id(CB_LABEL(cb_ref(x))));
}
...
void joutput_init_method(struct cb_program *prog) {
  ...
  if (gen_native) {
    ...
    joutput_line("f_native = CobolFieldFactory.makeCobolField(256, new "
...
void codegen(struct cb_program *prog, const int nested,
  ...
  *program_id_list = (char *)prog->program_id;
  ...
  if (gen_full_ebcdic) {
    ...
    i = lookup_attr(COB_TYPE_ALPHANUMERIC, 0, 0, 0, NULL, 0);
    joutput("  ");
    joutput("private static AbstractCobolField f_ebcdic = "
  ...
  if (gen_ebcdic_ascii) {
    ...
    i = lookup_attr(COB_TYPE_ALPHANUMERIC, 0, 0, 0, NULL, 0);
    joutput("  ");
    joutput("private static AbstractCobolField f_ebcdic_ascii = "
