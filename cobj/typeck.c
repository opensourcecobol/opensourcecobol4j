...
cb_check_numeric_name(cb_tree x)
{
	...
	if (CB_REFERENCE_P(x) && CB_FIELD_P(cb_ref(x)) && CB_TREE_CATEGORY(x) == CB_CATEGORY_NUMERIC)
	{
		return x;
	}
...
cb_build_section_name(cb_tree name, int sect_or_para)
{
	...
	if (CB_REFERENCE(name)->word->count > 0)
	{
		...
		/* Used as a non-label name or used as a section name.
		   ...
		   Checked in typeck.c */
		if (!CB_LABEL_P(x) || sect_or_para == 0 || (sect_or_para && CB_LABEL_P(x) && CB_LABEL(x)->is_section))
		{
...
int cb_reference_type_check(cb_tree ref, cb_tree x, const char *name, int size, int *retsize, int type)
{
			...
			if (p->flag_occurs)
			{
				...
				if (CB_EXCEPTION_ENABLE(COB_EC_BOUND_SUBSCRIPT))
				{
					if (p->occurs_depending)
					{
						...
						{
							e1 = cb_build_funcall_4("CobolCheck.checkOdo",
													cb_build_cast_integer(p->occurs_depending),
													cb_int(p->occurs_min),
													cb_int(p->occurs_max),
													cb_build_string0((ucharptr)(cb_field(p->occurs_depending)->name)));
		...
		if (CB_EXCEPTION_ENABLE(COB_EC_BOUND_REF_MOD))
		{
				...
				(r->length && !CB_LITERAL_P(r->length)))
			{
				e1 = cb_build_funcall_4("cob_check_ref_mod",
										...
										r->length ? cb_build_cast_integer(r->length) : cb_int1, cb_int(f->size),
				...
				(r->length && !CB_LITERAL_P(r->length)))
			{
					...
					cb_tree_category(CB_TREE(r)) == CB_CATEGORY_NATIONAL_EDITED)
				{
					e1 = cb_build_funcall_4("cob_check_ref_mod_national",
											cb_build_cast_integer(r->offset),
											r->length ? cb_build_cast_integer(r->length) : cb_int2, cb_int(f->size),
											cb_build_string0((ucharptr)f->name));
				}
				else
				{
...
expr_reduce(int token)
{
	...
	while (expr_prio[TOKEN(-2)] <= expr_prio[token])
	{
		...
		case '|':
			...
			{
				if ((CB_BINARY_OP_P(VALUE(-3)) && CB_BINARY_OP(VALUE(-3))->op == '&') || (CB_BINARY_OP_P(VALUE(-1)) && CB_BINARY_OP(VALUE(-1))->op == '&'))
				{
...
decimal_expand(cb_tree d, cb_tree x)
{
	...
	case CB_TAG_REFERENCE:
		...
		if (CB_EXCEPTION_ENABLE(COB_EC_DATA_INCOMPATIBLE))
		{
			...
			{
				dpush(cb_build_method_call_2("checkNumeric",
...
{
	dpush(cb_build_method_call_3("getField", d, x, build_store_option(x, round_opt)));
}
	...
	if (!CB_BINARY_OP_P(val))
	{
		if (op == '+' || op == '-')
		{
				...
				(CB_REF_OR_FIELD_P(val)))
			{
				...
				{
					cb_emit(cb_build_method_call_2("checkNumeric",
												   val,
												   cb_build_string0((ucharptr)(f->name))));
				}
			}
...
cb_build_optim_cond(struct cb_binary_op *p)
{
	...
	if (CB_REF_OR_FIELD_P(p->y))
	{
		...
		{
			return cb_build_method_call_2("cmpUint", p->x, cb_build_cast_integer(p->y));
		}
	}
	if (CB_REF_OR_FIELD_P(p->x))
	{
		...
		if (!f->pic->scale && f->usage == CB_USAGE_PACKED)
		{
			if (f->pic->digits < 10)
			{
				return cb_build_method_call_2("cmpInt",
											  p->x,
			...
			!f->flag_sign_leading && !f->flag_sign_separate)
		{
			if (cb_fits_int(p->x))
			{
				if (!f->pic->have_sign)
				{
					return cb_build_method_call_3("cmpNumdisp",
				...
				{
					return cb_build_method_call_3("cmpSignNumdisp",
												  cb_build_cast_address(p->x),
												  cb_int(f->size),
												  cb_build_cast_integer(p->y));
				}
			...
			else if (cb_fits_long_long(p->x))
			{
				if (!f->pic->have_sign)
				{
					return cb_build_method_call_3("cmpLongNumdisp",
												  cb_build_cast_address(p->x),
												  cb_int(f->size),
												  cb_build_cast_integer(p->y));
				}
				...
				{
					return cb_build_method_call_3("cmpLongSignNumdisp",
												  cb_build_cast_address(p->x),
												  cb_int(f->size),
												  cb_build_cast_integer(p->y));
				}
...
cb_build_cond(cb_tree x)
{
	...
	case CB_TAG_BINARY_OP:
		...
		default:
			if (CB_INDEX_P(p->x) || CB_INDEX_P(p->y) || CB_TREE_CLASS(p->x) == CB_CLASS_POINTER || CB_TREE_CLASS(p->y) == CB_CLASS_POINTER)
			{
			...
			else
			{
				...
				if (CB_TREE_CLASS(p->x) == CB_CLASS_NUMERIC && CB_TREE_CLASS(p->y) == CB_CLASS_NUMERIC && cb_fits_int(p->y))
				{
					x = cb_build_optim_cond(p);
					break;
				}
				if ((CB_REF_OR_FIELD_P(p->x)) && (CB_TREE_CATEGORY(p->x) == CB_CATEGORY_ALPHANUMERIC || CB_TREE_CATEGORY(p->x) == CB_CATEGORY_ALPHABETIC) && (cb_field_size(p->x) == 1) && (!current_program->alphabet_name_list) && (p->y == cb_space || p->y == cb_low || p->y == cb_high || p->y == cb_zero))
				{
							   ...
							   f->usage == CB_USAGE_COMP_X))
		{
			...
			if (s)
			{
				return cb_build_method_call_2(s,
				 ...
				 f->pic->digits < 10)
		{
			return cb_build_method_call_2("addPackedInt",
...
output_screen_from(struct cb_field *p, const size_t sisters)
{
	...
	type = (p->children ? COB_SCREEN_TYPE_GROUP : p->values	  ? COB_SCREEN_TYPE_VALUE
...
void cb_emit_close(cb_tree file, cb_tree opt)
{
	...
	cb_emit(cb_build_method_call_3("close", file, opt,
...
{
	cb_emit(cb_build_funcall_0("CobolFile.commit"));
}
...
void cb_emit_divide(cb_tree dividend, cb_tree divisor, cb_tree quotient, cb_tree remainder)
{
	...
	cb_emit(cb_build_method_call_4("divQuotient", dividend, divisor,
								   CB_VALUE(quotient),
								   build_store_option(CB_VALUE(quotient), CB_PURPOSE(quotient))));
	cb_emit(cb_build_method_call_2("divRemainder", CB_VALUE(remainder),
...
cb_validate_inspect_replaceable(cb_tree x, cb_tree y)
{
	...
	if (y == cb_zero || y == cb_space || y == cb_quote || y == cb_high || y == cb_low)
	{
		/* always replaceable */
	}
	...
	case CB_TAG_REFERENCE:
		...
		case CB_CATEGORY_NUMERIC:
			...
			case CB_CATEGORY_ALPHANUMERIC:
				...
				if (CB_TREE_CATEGORY(src) == CB_CATEGORY_NUMERIC && cb_field(src)->pic->scale > 0)
				{
				...
				if (CB_TREE_CATEGORY(src) == CB_CATEGORY_NUMERIC && cb_field(src)->pic->digits > dst_size_mod)
				{
					goto size_overflow_2;
				}
				if (CB_TREE_CATEGORY(src) == CB_CATEGORY_NUMERIC_EDITED && cb_field(src)->size > dst_size_mod)
				{
					goto size_overflow_1;
				}
				break;
			...
			default:
				src_scale_mod = cb_field(src)->pic->scale < 0 ? 0 : cb_field(src)->pic->scale;
				dst_scale_mod = cb_field(dst)->pic->scale < 0 ? 0 : cb_field(dst)->pic->scale;
...
cb_build_memset(cb_tree x, int c)
{
	...
	{
		return cb_build_method_call_3("fillBytes",
...
cb_build_move_copy(cb_tree src, cb_tree dst)
{
	...
	{
		return cb_build_method_call_3("setBytes",
...
cb_build_move_num_zero(cb_tree x)
{
	...
	case CB_USAGE_DISPLAY:
		...
		else if (cb_display_sign == COB_DISPLAY_SIGN_EBCDIC && f->pic->have_sign)
		{
			return cb_build_move_call(cb_zero, x);
		}
		else
...
cb_build_move_literal(cb_tree src, cb_tree dst)
{
	...
	else if ((cat == CB_CATEGORY_NUMERIC && f->usage == CB_USAGE_DISPLAY && f->pic->scale == l->scale && !f->flag_sign_leading && !f->flag_sign_separate) || ((cat == CB_CATEGORY_ALPHABETIC || cat == CB_CATEGORY_ALPHANUMERIC) && f->size < (int)(l->size + 16) && !cb_field_variable_size(f)))
	{
		buff = cobc_malloc((size_t)f->size);
...
cb_build_move_field(cb_tree src, cb_tree dst)
{
	...
	if ((src_size > 0 && dst_size > 0 && src_size >= dst_size) && (!cb_field_variable_size(src_f) && !cb_field_variable_size(dst_f)))
	{
		switch (CB_TREE_CATEGORY(src))
		{
		case CB_CATEGORY_ALPHABETIC:
			if (CB_TREE_CATEGORY(dst) == CB_CATEGORY_ALPHABETIC || CB_TREE_CATEGORY(dst) == CB_CATEGORY_ALPHANUMERIC)
			{
		...
		case CB_CATEGORY_ALPHANUMERIC:
			...
			break;
		case CB_CATEGORY_NUMERIC:
			if (CB_TREE_CATEGORY(dst) == CB_CATEGORY_NUMERIC && src_f->usage == dst_f->usage && src_f->pic->size == dst_f->pic->size && src_f->pic->digits == dst_f->pic->digits && src_f->pic->scale == dst_f->pic->scale && src_f->pic->have_sign == dst_f->pic->have_sign && src_f->flag_binary_swap == dst_f->flag_binary_swap && src_f->flag_sign_leading == dst_f->flag_sign_leading && src_f->flag_sign_separate == dst_f->flag_sign_separate)
			{
				return cb_build_move_copy(src, dst);
			}
			else if (CB_TREE_CATEGORY(dst) == CB_CATEGORY_ALPHANUMERIC && src_f->usage == CB_USAGE_DISPLAY && src_f->pic->have_sign == 0 && !src_f->flag_sign_leading && !src_f->flag_sign_separate)
			{
				return cb_build_move_copy(src, dst);
			}
			break;
...
void cb_emit_open(cb_tree file, cb_tree mode, cb_tree sharing)
{
	...
	cb_emit(cb_build_method_call_4("open", file, mode,
...
void cb_emit_read(cb_tree ref, cb_tree next, cb_tree into, cb_tree keys, cb_tree lock_opts)
{
	int read_opts = 0;
	cb_tree file;
	cb_tree rec;
		...
		CB_FILE(file)->access_mode == COB_ACCESS_SEQUENTIAL)
	{
		...
		cb_emit(cb_build_method_call_4("read", file, cb_int0,
									   CB_FILE(file)->file_status,
									   cb_int(read_opts)));
	}
	else
	{
		...
		/* READ */
		cb_emit(cb_build_method_call_4("read",
...
void cb_emit_rewrite(cb_tree record, cb_tree from, cb_tree lockopt)
{
	...
	cb_emit(cb_build_method_call_4("rewrite", file, record,
...
void cb_emit_set_to(cb_tree vars, cb_tree x)
{
	...
	for (l = vars; l; l = CB_CHAIN(l))
	{
		...
		{
			p = CB_CAST(v);
			if (p->type == CB_CAST_ADDRESS && !CB_FIELD(cb_ref(p->val))->flag_item_based && CB_FIELD(cb_ref(p->val))->storage != CB_STORAGE_LINKAGE)
			{
...
void cb_emit_sort_init(cb_tree name, cb_tree keys, cb_tree col)
{
	...
	if (CB_FILE_P(cb_ref(name)))
	{
		...
		cb_emit(cb_build_funcall_5("CobolFileSort.sortInit", cb_ref(name),
								   cb_int(cb_list_length(keys)), col,
								   cb_build_cast_address(current_program->cb_sort_return),
								   CB_FILE(cb_ref(name))->file_status));
		for (l = keys; l; l = CB_CHAIN(l))
	...
	else
	{
		...
		cb_emit(cb_build_funcall_2("CobolFileSort.sortTableInit", cb_int(cb_list_length(keys)), col));
		for (l = keys; l; l = CB_CHAIN(l))
		{
...
void cb_emit_sort_using(cb_tree file, cb_tree l)
{
	...
	for (; l; l = CB_CHAIN(l))
	{
		...
		cb_emit(cb_build_funcall_2("CobolFileSort.sortUsing",
...
{
	cb_emit(cb_build_funcall_1("CobolStopRunException.throwException", cb_build_cast_integer(x)));
}
...
void cb_emit_unlock(cb_tree ref)
{
	...
	{
		file = cb_ref(ref);
		cb_emit(cb_build_method_call_2("unlock",
