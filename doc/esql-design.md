...
The same suites also run on Windows through `.github/workflows/windows-test-esql.yml`, one job per suite. `esql-utf8` is excluded because the Windows build is Shift_JIS only.

There is no PostgreSQL container on Windows. The job runs [`win/start-test-postgresql.ps1`](../win/start-test-postgresql.ps1), which initialises a cluster with `initdb --encoding=UTF8 --no-locale` from the PostgreSQL that ships with the runner image and starts it on port 55432. The script takes the same steps on a developer's own Windows machine, reusing an existing cluster if it finds one; `-Stop` shuts it down again. Its comment header explains why `--no-locale` and the non-default port are both required.

The connection settings the test programs are compiled with come from `.github/workflows/db-settings/`: the Windows job copies `embed_db_info_windows.sh` over `tests/embed_db_info.sh`, and `win/start-test-postgresql.ps1` reads its own default port, database and role out of that same file, so the server and the programs cannot disagree. The `embed_db_info_postgresql_*.sh` files next to it are the equivalent for the container hosts, but the Linux workflows cannot use them — those jobs run from the `make dist` tarball with no `actions/checkout`, and `.github/` is not part of the tarball, so they write the same script inline instead.
