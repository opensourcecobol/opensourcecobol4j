...
joutput_line (const char *fmt, ...)
{
	...
	if(joutput_target) {
		if(joutput_buffered) {
			joutput_prefix ();
			//本当は以下のように書きたいがうまく行かないので
			//va_start (ap, fmt);
			//joutput (fmt, ap);
			//va_end (ap);

			// joutputのコードを直接引っ張ってきた
			va_start (ap, fmt);
			size = vsprintf (joutput_temp_buffer, fmt, ap);
			va_end (ap);
...
static void
joutput_base (struct cb_field *f)
{
	struct cb_field		*f01;
	struct cb_field		*p;
	struct cb_field		*v;
...
static void
joutput_func_1 (const char *name, cb_tree x)
{
	joutput ("%s (", name);
	joutput_param (x, param_id);
	joutput (")");
...
static void
joutput_cond (cb_tree x, int save_flag)
{
	struct cb_binary_op	*p;
...
static void
joutput_move (cb_tree src, cb_tree dst)
{
	/* suppress warnings */
	suppress_warn = 1;
...
static void
joutput_figurative (cb_tree x, struct cb_field *f, const int value)
{
	joutput_prefix ();
	if (f->size == 1) {
		joutput ("*(unsigned char *)(");
...
static void
joutput_initialize_literal (cb_tree x, struct cb_field *f, struct cb_literal *l)
{
	size_t	i;
	size_t	n;
...
static void
joutput_initialize_fp (cb_tree x, struct cb_field *f)
{
	joutput_prefix ();
...
static void
joutput_initialize_external (cb_tree x, struct cb_field *f)
{
	unsigned char	*p;
	cb_tree		file;
	char		name[COB_MINI_BUFF];
...
static void
joutput_initialize_uniform (cb_tree x, int c, int size)
{
	joutput_prefix ();
...
static void
joutput_initialize_one (struct cb_initialize *p, cb_tree x)
{
	struct cb_field		*f;
	cb_tree			value;
	cb_tree			lrp;
...
static void
joutput_initialize_compound (struct cb_initialize *p, cb_tree x)
{
	struct cb_field	*ff;
	struct cb_field	*f;
	...
	while(f) {
		...
		default:
			if (f->flag_occurs) {
				...
				i = f->indexes;
				i_counters[i] = 1;
				joutput_line ("for (int i%d = 1; i%d <= %d; i%d++)",
					     i, i, f->occurs_max, i);
				joutput_indent ("  {");
				CB_REFERENCE (c)->subs =
...
static void
joutput_initialize (struct cb_initialize *p)
{
	struct cb_field *f;
	int		c;
...
static void
joutput_occurs (struct cb_field *p)
{
...
static void
joutput_search_whens (cb_tree table, cb_tree var, cb_tree stmt, cb_tree whens)
{
	cb_tree		l;
	struct cb_field *p;
	cb_tree		idx = NULL;
...
static void
joutput_search_all (cb_tree table, cb_tree stmt, cb_tree cond, cb_tree when)
{
	struct cb_field *p;
	cb_tree		idx;
...
static void
joutput_search (struct cb_search *p)
{
	if (p->flag_all) {
		joutput_search_all (p->table, p->end_stmt,
...
joutput_call (struct cb_call *p)
{
	...
	if (!dynamic_link) {
		...
		if (CB_LITERAL_P (p->name)) {
			...
			joutput_prefix ();
			joutput ("  cob_unifunc = ");
			joutput_funcall (cb_build_funcall_1 (
					"CobolResolve.resolve", p->name));
			joutput (";\n");
		}
		// joutput_prefix ();
	...
	if (p->returning) {
		if (!retptr) {
			/* suppress warnings */
			suppress_warn = 1;
			joutput_stmt (cb_build_move (current_prog->cb_return_code,
						    p->returning));
			suppress_warn = 0;
#ifdef	COB_NON_ALIGNED
		} else {
...
static void
joutput_perform_call (struct cb_label *lb, struct cb_label *le)
{
#ifndef	__GNUC__
	struct label_list *l;
#endif
...
static void
joutput_perform_exit (struct cb_label *l)
{
	if (l->is_global) {
		joutput_newline ();
...
//#endif
		...
		joutput_line ("  if (temp_index->perform_through == %d) {", l->id);
		joutput_line ("    frame_ptr = temp_index;");
#ifndef	__GNUC__
		joutput_line ("    goto P_switch;");
#elif	COB_USE_SETJMP
		joutput_line ("    longjmp (frame_ptr->return_address, 1);");
...
static void
joutput_perform_once (struct cb_perform *p)
{
...
static void
joutput_perform_until (struct cb_perform *p, cb_tree l)
{
...
static void
joutput_perform (struct cb_perform *p)
{
	struct cb_perform_varying *v;
...
static void
joutput_sort_init (struct cb_sort_init *p)
{
	joutput_prefix ();
	joutput ("%s (", p->name);
...
static void
joutput_sort_proc (struct cb_sort_proc *p)
{
...
static void
joutput_sort_finish (struct cb_sort_finish *p)
{
	if (p->sort_file != NULL) {
		joutput_prefix ();
...
static void
joutput_file_error (struct cb_file *pfile)
{
	struct cb_file		*fl;
	cb_tree			l;
	...
	for (l =  current_prog->local_file_list; l; l = CB_CHAIN (l)) {
		...
		if (!strcmp (pfile->name, fl->name)) {
			joutput_perform_call (fl->handler,
					     fl->handler);
			return;
		}
	}
...
static void
joutput_ferror_stmt (struct cb_statement *p, int code)
{
...
joutput_stmt (cb_tree x)
{
	...
	case CB_TAG_LABEL:
		...
		if (lp->need_begin) {
			...
			//int tmp = joutput_buffered;
			//joutput_buffered = 0;
			//joutput_line ("LABEL_%d();", lp->id);
			//joutput_buffered = tmp;

			//JavaではLabelを使用しないので削除した
			//joutput_newline ();
			//joutput_line ("%s%d:;", CB_PREFIX_LABEL, lp->id);
		}
		if (cb_flag_trace) {
			if (lp->is_section) {
				if (strcmp ((const char *)(lp->name) , "MAIN SECTION")) {
...
static int
joutput_file_allocation (struct cb_file *f)
{
...
static void
joutput_file_initialization (struct cb_file *f)
{
	int			nkeys = 1;
	int			i_keycomp;
	...
	if (f->organization == COB_ORG_RELATIVE
	 || f->organization == COB_ORG_INDEXED) {
		...
		joutput_indent ("{");
		/*joutput_line ("%s%s = cob_malloc (sizeof (struct cob_file_key) * %d);",
			     CB_PREFIX_KEYS, f->cname, nkeys);*/
		joutput_line ("%s%s = new CobolFileKey[%d];",
			     CB_PREFIX_KEYS, f->cname, nkeys);
		...
		if (f->key) {
			if (f->component_list != NULL) {
				...
				for (key_component = f->component_list, i_keycomp = 0;
					...
					joutput_prefix ();
					joutput ("%s%s[0].getComponent()[%d].field = ", CB_PREFIX_KEYS, f->cname, i_keycomp);
		...
		for (l = f->alt_key_list; l; l = l->next) {
			joutput_prefix ();
			joutput ("%s%s[%d].setField(", CB_PREFIX_KEYS, f->cname,
				nkeys);
			...
			if (l->component_list != NULL) {
				for (key_component = l->component_list, i_keycomp = 0;
				     ...
				     key_component = key_component->next, ++i_keycomp) {
					joutput_prefix ();
					joutput ("%s%s[%d].getComponent()[%d].field = ", CB_PREFIX_KEYS, f->cname, nkeys, i_keycomp);
					joutput_param (key_component->component, -1);
					joutput (";\n");
					joutput_prefix ();
	...
	joutput_line("%s%s = CobolFileFactory.makeCobolFileInstance(", CB_PREFIX_FILE, f->cname);
	...
	if (f->linage) {
		joutput_line ("lingptr = (Linage)(%s%s.getLinorkeyptr());",
				CB_PREFIX_FILE, f->cname);
		joutput_prefix ();
...
static void
joutput_initial_values (struct cb_field *p)
{
	cb_tree	x;
	cb_tree	def;
...
joutput_internal_function (struct cb_program *prog, cb_tree parameter_list)
{
	...
	//joutput_line ("if (!this.initialized)");
	//joutput_indent ("  {");
	//joutput_line ("if (!this.cobolInitialized) {");
	//if (cb_flag_implicit_init) {
	//	joutput_line ("  cob_init (0, NULL);");
	//} else {
	//	joutput_line ("  this.cobolFatalError (Const.COB_FERROR_INITIALIZED);");
	//}
	//joutput_line ("}");
	//output_line
	//    ("cobolCheckVersion (Const.COB_SOURCE_FILE, Const.COB_PACKAGE_VERSION, Const.COB_PATCH_LEVEL);");
	//if (!prog->flag_main) {
	//	if (cb_flag_implicit_init) {
	...
	output_line ("CobolModule.pollLast();");
	output_newline ();
	//if (cb_flag_traceall) {
	...
	//	joutput_newline ();
	//	if (seen) {
	//		joutput_line ("switch (cob_error_file->last_open_mode)");
	//		joutput_indent ("{");
	//		for (i = COB_OPEN_INPUT; i <= COB_OPEN_EXTEND; i++) {
	...
	//		joutput_indent ("{");
	//	}
	//	joutput_line ("if (!(cob_error_file->flag_select_features & COB_SELECT_FILE_STATUS)) {");
	//	switch (cb_abort_on_io_exception) {
	//	case CB_ABORT_ON_IO_ANY:
	//		joutput_line ("	cob_default_error_handle ();");
	//		joutput_line ("	cob_stop_run (1);");
	//		break;
	//	case CB_ABORT_ON_IO_FATAL:
	//		joutput_line ("	if (cob_error_file->file_status[0] == '3'");
	//		joutput_line ("	    || cob_error_file->file_status[0] == '4'");
	//		joutput_line ("	    || cob_error_file->file_status[0] == '9') {");
	//		joutput_line ("		cob_default_error_handle ();");
	//		joutput_line ("		cob_stop_run (1);");
	//		joutput_line ("	}");
	//		break;
	...
	//	joutput_newline ();
	//	joutput_line ("cob_fatal_error (COB_FERROR_CODEGEN);");
	//	joutput_newline ();
	//}	
	joutput_buffered = 0;
#ifndef	__GNUC__
...
void joutput_init_method(struct cb_program *prog) {
	...
	if (attr_cache) {
		...
		for (j = attr_cache; j; j = j->next) {
			...
			joutput ("%s%d = ",
					CB_PREFIX_ATTR, j->id);
			joutput ("new CobolFieldAttribute (%d, %d, %d, %d, ",
					j->type, j->digits,
					j->scale, j->flags);
			if (j->pic) {
				joutput ("\"");
...
codegen (struct cb_program *prog, const int nested)
{
	...
	joutput_line("private void cobolCheckVersion(String sourceFile, int packageVersion, int patchVersion)");
	joutput_line("{");
	joutput_line("}\n");

	joutput_line("private void cobolSetCancel(String programId, Object a, Object b)");
	joutput_line("{");
	joutput_line("}\n");
	...
	joutput_line("private static CobolDataStorage makeCobolDataStorage(byte ...bytes) {");
	...
	if (gen_ebcdic) {
	    ...
	    joutput_indent_level += 2;
		if (alt_ebcdic) {
			joutput_indent ("(byte)0x00, (byte)0x01, (byte)0x02, (byte)0x03, (byte)0x37, (byte)0x2D, (byte)0x2E, (byte)0x2F,");
			joutput_indent ("(byte)0x16, (byte)0x05, (byte)0x25, (byte)0x0B, (byte)0x0C, (byte)0x0D, (byte)0x0E, (byte)0x0F,");
			joutput_indent ("(byte)0x10, (byte)0x11, (byte)0x12, (byte)0x13, (byte)0x3C, (byte)0x3D, (byte)0x32, (byte)0x26,");
			joutput_indent ("(byte)0x18, (byte)0x19, (byte)0x3F, (byte)0x27, (byte)0x1C, (byte)0x1D, (byte)0x1E, (byte)0x1F,");
			joutput_indent ("(byte)0x40, (byte)0x5A, (byte)0x7F, (byte)0x7B, (byte)0x5B, (byte)0x6C, (byte)0x50, (byte)0x7D,");
			joutput_indent ("(byte)0x4D, (byte)0x5D, (byte)0x5C, (byte)0x4E, (byte)0x6B, (byte)0x60, (byte)0x4B, (byte)0x61,");
			joutput_indent ("(byte)0xF0, (byte)0xF1, (byte)0xF2, (byte)0xF3, (byte)0xF4, (byte)0xF5, (byte)0xF6, (byte)0xF7,");
			joutput_indent ("(byte)0xF8, (byte)0xF9, (byte)0x7A, (byte)0x5E, (byte)0x4C, (byte)0x7E, (byte)0x6E, (byte)0x6F,");
			joutput_indent ("(byte)0x7C, (byte)0xC1, (byte)0xC2, (byte)0xC3, (byte)0xC4, (byte)0xC5, (byte)0xC6, (byte)0xC7,");
			joutput_indent ("(byte)0xC8, (byte)0xC9, (byte)0xD1, (byte)0xD2, (byte)0xD3, (byte)0xD4, (byte)0xD5, (byte)0xD6,");
			joutput_indent ("(byte)0xD7, (byte)0xD8, (byte)0xD9, (byte)0xE2, (byte)0xE3, (byte)0xE4, (byte)0xE5, (byte)0xE6,");
			joutput_indent ("(byte)0xE7, (byte)0xE8, (byte)0xE9, (byte)0xAD, (byte)0xE0, (byte)0xBD, (byte)0x5F, (byte)0x6D,");
			joutput_indent ("(byte)0x79, (byte)0x81, (byte)0x82, (byte)0x83, (byte)0x84, (byte)0x85, (byte)0x86, (byte)0x87,");
			joutput_indent ("(byte)0x88, (byte)0x89, (byte)0x91, (byte)0x92, (byte)0x93, (byte)0x94, (byte)0x95, (byte)0x96,");
			joutput_indent ("(byte)0x97, (byte)0x98, (byte)0x99, (byte)0xA2, (byte)0xA3, (byte)0xA4, (byte)0xA5, (byte)0xA6,");
			joutput_indent ("(byte)0xA7, (byte)0xA8, (byte)0xA9, (byte)0xC0, (byte)0x6A, (byte)0xD0, (byte)0xA1, (byte)0x07,");
			joutput_indent ("(byte)0x68, (byte)0xDC, (byte)0x51, (byte)0x42, (byte)0x43, (byte)0x44, (byte)0x47, (byte)0x48,");
			joutput_indent ("(byte)0x52, (byte)0x53, (byte)0x54, (byte)0x57, (byte)0x56, (byte)0x58, (byte)0x63, (byte)0x67,");
			joutput_indent ("(byte)0x71, (byte)0x9C, (byte)0x9E, (byte)0xCB, (byte)0xCC, (byte)0xCD, (byte)0xDB, (byte)0xDD,");
			joutput_indent ("(byte)0xDF, (byte)0xEC, (byte)0xFC, (byte)0xB0, (byte)0xB1, (byte)0xB2, (byte)0x3E, (byte)0xB4,");
			joutput_indent ("(byte)0x45, (byte)0x55, (byte)0xCE, (byte)0xDE, (byte)0x49, (byte)0x69, (byte)0x9A, (byte)0x9B,");
			joutput_indent ("(byte)0xAB, (byte)0x9F, (byte)0xBA, (byte)0xB8, (byte)0xB7, (byte)0xAA, (byte)0x8A, (byte)0x8B,");
			joutput_indent ("(byte)0xB6, (byte)0xB5, (byte)0x62, (byte)0x4F, (byte)0x64, (byte)0x65, (byte)0x66, (byte)0x20,");
			joutput_indent ("(byte)0x21, (byte)0x22, (byte)0x70, (byte)0x23, (byte)0x72, (byte)0x73, (byte)0x74, (byte)0xBE,");
			joutput_indent ("(byte)0x76, (byte)0x77, (byte)0x78, (byte)0x80, (byte)0x24, (byte)0x15, (byte)0x8C, (byte)0x8D,");
			joutput_indent ("(byte)0x8E, (byte)0x41, (byte)0x06, (byte)0x17, (byte)0x28, (byte)0x29, (byte)0x9D, (byte)0x2A,");
			joutput_indent ("(byte)0x2B, (byte)0x2C, (byte)0x09, (byte)0x0A, (byte)0xAC, (byte)0x4A, (byte)0xAE, (byte)0xAF,");
			joutput_indent ("(byte)0x1B, (byte)0x30, (byte)0x31, (byte)0xFA, (byte)0x1A, (byte)0x33, (byte)0x34, (byte)0x35,");
			joutput_indent ("(byte)0x36, (byte)0x59, (byte)0x08, (byte)0x38, (byte)0xBC, (byte)0x39, (byte)0xA0, (byte)0xBF,");
			joutput_indent ("(byte)0xCA, (byte)0x3A, (byte)0xFE, (byte)0x3B, (byte)0x04, (byte)0xCF, (byte)0xDA, (byte)0x14,");
			joutput_indent ("(byte)0xE1, (byte)0x8F, (byte)0x46, (byte)0x75, (byte)0xFD, (byte)0xEB, (byte)0xEE, (byte)0xED,");
			joutput_indent ("(byte)0x90, (byte)0xEF, (byte)0xB3, (byte)0xFB, (byte)0xB9, (byte)0xEA, (byte)0xBB, (byte)0xFF");
		} else {
			/* MF */
			joutput_indent ("(byte)0x00, (byte)0x01, (byte)0x02, (byte)0x03, (byte)0x1D, (byte)0x19, (byte)0x1A, (byte)0x1B,");
			joutput_indent ("(byte)0x0F, (byte)0x04, (byte)0x16, (byte)0x06, (byte)0x07, (byte)0x08, (byte)0x09, (byte)0x0A,");
			joutput_indent ("(byte)0x0B, (byte)0x0C, (byte)0x0D, (byte)0x0E, (byte)0x1E, (byte)0x1F, (byte)0x1C, (byte)0x17,");
			joutput_indent ("(byte)0x10, (byte)0x11, (byte)0x20, (byte)0x18, (byte)0x12, (byte)0x13, (byte)0x14, (byte)0x15,");
			joutput_indent ("(byte)0x21, (byte)0x27, (byte)0x3A, (byte)0x36, (byte)0x28, (byte)0x30, (byte)0x26, (byte)0x38,");
			joutput_indent ("(byte)0x24, (byte)0x2A, (byte)0x29, (byte)0x25, (byte)0x2F, (byte)0x2C, (byte)0x22, (byte)0x2D,");
			joutput_indent ("(byte)0x73, (byte)0x74, (byte)0x75, (byte)0x76, (byte)0x77, (byte)0x78, (byte)0x79, (byte)0x7A,");
			joutput_indent ("(byte)0x7B, (byte)0x7C, (byte)0x35, (byte)0x2B, (byte)0x23, (byte)0x39, (byte)0x32, (byte)0x33,");
			joutput_indent ("(byte)0x37, (byte)0x57, (byte)0x58, (byte)0x59, (byte)0x5A, (byte)0x5B, (byte)0x5C, (byte)0x5D,");
			joutput_indent ("(byte)0x5E, (byte)0x5F, (byte)0x61, (byte)0x62, (byte)0x63, (byte)0x64, (byte)0x65, (byte)0x66,");
			joutput_indent ("(byte)0x67, (byte)0x68, (byte)0x69, (byte)0x6B, (byte)0x6C, (byte)0x6D, (byte)0x6E, (byte)0x6F,");
			joutput_indent ("(byte)0x70, (byte)0x71, (byte)0x72, (byte)0x7D, (byte)0x6A, (byte)0x7E, (byte)0x7F, (byte)0x31,");
			joutput_indent ("(byte)0x34, (byte)0x3B, (byte)0x3C, (byte)0x3D, (byte)0x3E, (byte)0x3F, (byte)0x40, (byte)0x41,");
			joutput_indent ("(byte)0x42, (byte)0x43, (byte)0x44, (byte)0x45, (byte)0x46, (byte)0x47, (byte)0x48, (byte)0x49,");
			joutput_indent ("(byte)0x4A, (byte)0x4B, (byte)0x4C, (byte)0x4E, (byte)0x4F, (byte)0x50, (byte)0x51, (byte)0x52,");
			joutput_indent ("(byte)0x53, (byte)0x54, (byte)0x55, (byte)0x56, (byte)0x2E, (byte)0x60, (byte)0x4D, (byte)0x05,");
			joutput_indent ("(byte)0x80, (byte)0x81, (byte)0x82, (byte)0x83, (byte)0x84, (byte)0x85, (byte)0x86, (byte)0x87,");
			joutput_indent ("(byte)0x88, (byte)0x89, (byte)0x8A, (byte)0x8B, (byte)0x8C, (byte)0x8D, (byte)0x8E, (byte)0x8F,");
			joutput_indent ("(byte)0x90, (byte)0x91, (byte)0x92, (byte)0x93, (byte)0x94, (byte)0x95, (byte)0x96, (byte)0x97,");
			joutput_indent ("(byte)0x98, (byte)0x99, (byte)0x9A, (byte)0x9B, (byte)0x9C, (byte)0x9D, (byte)0x9E, (byte)0x9F,");
			joutput_indent ("(byte)0xA0, (byte)0xA1, (byte)0xA2, (byte)0xA3, (byte)0xA4, (byte)0xA5, (byte)0xA6, (byte)0xA7,");
			joutput_indent ("(byte)0xA8, (byte)0xA9, (byte)0xAA, (byte)0xAB, (byte)0xAC, (byte)0xAD, (byte)0xAE, (byte)0xAF,");
			joutput_indent ("(byte)0xB0, (byte)0xB1, (byte)0xB2, (byte)0xB3, (byte)0xB4, (byte)0xB5, (byte)0xB6, (byte)0xB7,");
			joutput_indent ("(byte)0xB8, (byte)0xB9, (byte)0xBA, (byte)0xBB, (byte)0xBC, (byte)0xBD, (byte)0xBE, (byte)0xBF,");
			joutput_indent ("(byte)0xC0, (byte)0xC1, (byte)0xC2, (byte)0xC3, (byte)0xC4, (byte)0xC5, (byte)0xC6, (byte)0xC7,");
			joutput_indent ("(byte)0xC8, (byte)0xC9, (byte)0xCA, (byte)0xCB, (byte)0xCC, (byte)0xCD, (byte)0xCE, (byte)0xCF,");
			joutput_indent ("(byte)0xD0, (byte)0xD1, (byte)0xD2, (byte)0xD3, (byte)0xD4, (byte)0xD5, (byte)0xD6, (byte)0xD7,");
			joutput_indent ("(byte)0xD8, (byte)0xD9, (byte)0xDA, (byte)0xDB, (byte)0xDC, (byte)0xDD, (byte)0xDE, (byte)0xDF,");
			joutput_indent ("(byte)0xE0, (byte)0xE1, (byte)0xE2, (byte)0xE3, (byte)0xE4, (byte)0xE5, (byte)0xE6, (byte)0xE7,");
			joutput_indent ("(byte)0xE8, (byte)0xE9, (byte)0xEA, (byte)0xEB, (byte)0xEC, (byte)0xED, (byte)0xEE, (byte)0xEF,");
			joutput_indent ("(byte)0xF0, (byte)0xF1, (byte)0xF2, (byte)0xF3, (byte)0xF4, (byte)0xF5, (byte)0xF6, (byte)0xF7,");
			joutput_indent ("(byte)0xF8, (byte)0xF9, (byte)0xFA, (byte)0xFB, (byte)0xFC, (byte)0xFD, (byte)0xFE, (byte)0xFF");
		}
	...
	if (gen_full_ebcdic) {
        ...
        joutput("  ");
		joutput
		    ("private static AbstractCobolField f_ebcdic = CobolFieldFactori.makeField(256, new CobolDataStorage(cob_ebcdic), %s%d);\n",
		     CB_PREFIX_ATTR, i);
		joutput ("\n");
	}
	if (gen_ebcdic_ascii) {
	    ...
	    joutput_indent_level += 2;
		joutput_indent ("(byte)0x00, (byte)0x01, (byte)0x02, (byte)0x03, (byte)0xEC, (byte)0x09, (byte)0xCA, (byte)0x7F,");
		joutput_indent ("(byte)0xE2, (byte)0xD2, (byte)0xD3, (byte)0x0B, (byte)0x0C, (byte)0x0D, (byte)0x0E, (byte)0x0F,");
		joutput_indent ("(byte)0x10, (byte)0x11, (byte)0x12, (byte)0x13, (byte)0xEF, (byte)0xC5, (byte)0x08, (byte)0xCB,");
		joutput_indent ("(byte)0x18, (byte)0x19, (byte)0xDC, (byte)0xD8, (byte)0x1C, (byte)0x1D, (byte)0x1E, (byte)0x1F,");
		joutput_indent ("(byte)0xB7, (byte)0xB8, (byte)0xB9, (byte)0xBB, (byte)0xC4, (byte)0x0A, (byte)0x17, (byte)0x1B,");
		joutput_indent ("(byte)0xCC, (byte)0xCD, (byte)0xCF, (byte)0xD0, (byte)0xD1, (byte)0x05, (byte)0x06, (byte)0x07,");
		joutput_indent ("(byte)0xD9, (byte)0xDA, (byte)0x16, (byte)0xDD, (byte)0xDE, (byte)0xDF, (byte)0xE0, (byte)0x04,");
		joutput_indent ("(byte)0xE3, (byte)0xE5, (byte)0xE9, (byte)0xEB, (byte)0x14, (byte)0x15, (byte)0x9E, (byte)0x1A,");
		joutput_indent ("(byte)0x20, (byte)0xC9, (byte)0x83, (byte)0x84, (byte)0x85, (byte)0xA0, (byte)0xF2, (byte)0x86,");
		joutput_indent ("(byte)0x87, (byte)0xA4, (byte)0xD5, (byte)0x2E, (byte)0x3C, (byte)0x28, (byte)0x2B, (byte)0xB3,");
		joutput_indent ("(byte)0x26, (byte)0x82, (byte)0x88, (byte)0x89, (byte)0x8A, (byte)0xA1, (byte)0x8C, (byte)0x8B,");
		joutput_indent ("(byte)0x8D, (byte)0xE1, (byte)0x21, (byte)0x24, (byte)0x2A, (byte)0x29, (byte)0x3B, (byte)0x5E,");
		joutput_indent ("(byte)0x2D, (byte)0x2F, (byte)0xB2, (byte)0x8E, (byte)0xB4, (byte)0xB5, (byte)0xB6, (byte)0x8F,");
		joutput_indent ("(byte)0x80, (byte)0xA5, (byte)0x7C, (byte)0x2C, (byte)0x25, (byte)0x5F, (byte)0x3E, (byte)0x3F,");
		joutput_indent ("(byte)0xBA, (byte)0x90, (byte)0xBC, (byte)0xBD, (byte)0xBE, (byte)0xF3, (byte)0xC0, (byte)0xC1,");
		joutput_indent ("(byte)0xC2, (byte)0x60, (byte)0x3A, (byte)0x23, (byte)0x40, (byte)0x27, (byte)0x3D, (byte)0x22,");
		joutput_indent ("(byte)0xC3, (byte)0x61, (byte)0x62, (byte)0x63, (byte)0x64, (byte)0x65, (byte)0x66, (byte)0x67,");
		joutput_indent ("(byte)0x68, (byte)0x69, (byte)0xAE, (byte)0xAF, (byte)0xC6, (byte)0xC7, (byte)0xC8, (byte)0xF1,");
		joutput_indent ("(byte)0xF8, (byte)0x6A, (byte)0x6B, (byte)0x6C, (byte)0x6D, (byte)0x6E, (byte)0x6F, (byte)0x70,");
		joutput_indent ("(byte)0x71, (byte)0x72, (byte)0xA6, (byte)0xA7, (byte)0x91, (byte)0xCE, (byte)0x92, (byte)0xA9,");
		joutput_indent ("(byte)0xE6, (byte)0x7E, (byte)0x73, (byte)0x74, (byte)0x75, (byte)0x76, (byte)0x77, (byte)0x78,");
		joutput_indent ("(byte)0x79, (byte)0x7A, (byte)0xAD, (byte)0xA8, (byte)0xD4, (byte)0x5B, (byte)0xD6, (byte)0xD7,");
		joutput_indent ("(byte)0x9B, (byte)0x9C, (byte)0x9D, (byte)0xFA, (byte)0x9F, (byte)0xB1, (byte)0xB0, (byte)0xAC,");
		joutput_indent ("(byte)0xAB, (byte)0xFC, (byte)0xAA, (byte)0xFE, (byte)0xE4, (byte)0x5D, (byte)0xBF, (byte)0xE7,");
		joutput_indent ("(byte)0x7B, (byte)0x41, (byte)0x42, (byte)0x43, (byte)0x44, (byte)0x45, (byte)0x46, (byte)0x47,");
		joutput_indent ("(byte)0x48, (byte)0x49, (byte)0xE8, (byte)0x93, (byte)0x94, (byte)0x95, (byte)0xA2, (byte)0xED,");
		joutput_indent ("(byte)0x7D, (byte)0x4A, (byte)0x4B, (byte)0x4C, (byte)0x4D, (byte)0x4E, (byte)0x4F, (byte)0x50,");
		joutput_indent ("(byte)0x51, (byte)0x52, (byte)0xEE, (byte)0x96, (byte)0x81, (byte)0x97, (byte)0xA3, (byte)0x98,");
		joutput_indent ("(byte)0x5C, (byte)0xF0, (byte)0x53, (byte)0x54, (byte)0x55, (byte)0x56, (byte)0x57, (byte)0x58,");
		joutput_indent ("(byte)0x59, (byte)0x5A, (byte)0xFD, (byte)0xF5, (byte)0x99, (byte)0xF7, (byte)0xF6, (byte)0xF9,");
		joutput_indent ("(byte)0x30, (byte)0x31, (byte)0x32, (byte)0x33, (byte)0x34, (byte)0x35, (byte)0x36, (byte)0x37,");
		joutput_indent ("(byte)0x38, (byte)0x39, (byte)0xDB, (byte)0xFB, (byte)0x9A, (byte)0xF4, (byte)0xEA, (byte)0xFF");
        ...
        joutput("  ");
		joutput
		      ("private static AbstractCobolField f_ebcdic_ascii = CobolFieldFactory.makeField(256, new CobolDataStorage(cob_ebcdic_ascii), %s%d);\n",
		       CB_PREFIX_ATTR, i);
		joutput ("\n");
	}
	if (gen_native) {
	    ...
	    joutput_indent_level += 2;
		joutput_indent ("(byte)0, (byte)1, (byte)2, (byte)3, (byte)4, (byte)5, (byte)6, (byte)7,");
		joutput_indent ("(byte)8, (byte)9, (byte)10, (byte)11, (byte)12, (byte)13, (byte)14, (byte)15,");
		joutput_indent ("(byte)16, (byte)17, (byte)18, (byte)19, (byte)20, (byte)21, (byte)22, (byte)23,");
		joutput_indent ("(byte)24, (byte)25, (byte)26, (byte)27, (byte)28, (byte)29, (byte)30, (byte)31,");
		joutput_indent ("(byte)32, (byte)33, (byte)34, (byte)35, (byte)36, (byte)37, (byte)38, (byte)39,");
		joutput_indent ("(byte)40, (byte)41, (byte)42, (byte)43, (byte)44, (byte)45, (byte)46, (byte)47,");
		joutput_indent ("(byte)48, (byte)49, (byte)50, (byte)51, (byte)52, (byte)53, (byte)54, (byte)55,");
		joutput_indent ("(byte)56, (byte)57, (byte)58, (byte)59, (byte)60, (byte)61, (byte)62, (byte)63,");
		joutput_indent ("(byte)64, (byte)65, (byte)66, (byte)67, (byte)68, (byte)69, (byte)70, (byte)71,");
		joutput_indent ("(byte)72, (byte)73, (byte)74, (byte)75, (byte)76, (byte)77, (byte)78, (byte)79,");
		joutput_indent ("(byte)80, (byte)81, (byte)82, (byte)83, (byte)84, (byte)85, (byte)86, (byte)87,");
		joutput_indent ("(byte)88, (byte)89, (byte)90, (byte)91, (byte)92, (byte)93, (byte)94, (byte)95,");
		joutput_indent ("(byte)96, (byte)97, (byte)98, (byte)99, (byte)100, (byte)101, (byte)102, (byte)103,");
		joutput_indent ("(byte)104, (byte)105, (byte)106, (byte)107, (byte)108, (byte)109, (byte)110, (byte)111,");
		joutput_indent ("(byte)112, (byte)113, (byte)114, (byte)115, (byte)116, (byte)117, (byte)118, (byte)119,");
		joutput_indent ("(byte)120, (byte)121, (byte)122, (byte)123, (byte)124, (byte)125, (byte)126, (byte)127,");
		joutput_indent ("(byte)128, (byte)129, (byte)130, (byte)131, (byte)132, (byte)133, (byte)134, (byte)135,");
		joutput_indent ("(byte)136, (byte)137, (byte)138, (byte)139, (byte)140, (byte)141, (byte)142, (byte)143,");
		joutput_indent ("(byte)144, (byte)145, (byte)146, (byte)147, (byte)148, (byte)149, (byte)150, (byte)151,");
		joutput_indent ("(byte)152, (byte)153, (byte)154, (byte)155, (byte)156, (byte)157, (byte)158, (byte)159,");
		joutput_indent ("(byte)160, (byte)161, (byte)162, (byte)163, (byte)164, (byte)165, (byte)166, (byte)167,");
		joutput_indent ("(byte)168, (byte)169, (byte)170, (byte)171, (byte)172, (byte)173, (byte)174, (byte)175,");
		joutput_indent ("(byte)176, (byte)177, (byte)178, (byte)179, (byte)180, (byte)181, (byte)182, (byte)183,");
		joutput_indent ("(byte)184, (byte)185, (byte)186, (byte)187, (byte)188, (byte)189, (byte)190, (byte)191,");
		joutput_indent ("(byte)192, (byte)193, (byte)194, (byte)195, (byte)196, (byte)197, (byte)198, (byte)199,");
		joutput_indent ("(byte)200, (byte)201, (byte)202, (byte)203, (byte)204, (byte)205, (byte)206, (byte)207,");
		joutput_indent ("(byte)208, (byte)209, (byte)210, (byte)211, (byte)212, (byte)213, (byte)214, (byte)215,");
		joutput_indent ("(byte)216, (byte)217, (byte)218, (byte)219, (byte)220, (byte)221, (byte)222, (byte)223,");
		joutput_indent ("(byte)224, (byte)225, (byte)226, (byte)227, (byte)228, (byte)229, (byte)230, (byte)231,");
		joutput_indent ("(byte)232, (byte)233, (byte)234, (byte)235, (byte)236, (byte)237, (byte)238, (byte)239,");
		joutput_indent ("(byte)240, (byte)241, (byte)242, (byte)243, (byte)244, (byte)245, (byte)246, (byte)247,");
		joutput_indent ("(byte)248, (byte)249, (byte)250, (byte)251, (byte)252, (byte)253, (byte)254, (byte)255");
